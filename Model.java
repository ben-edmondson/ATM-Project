...
public class Model {
    ...
    // Number used to designate functions
    int funcNum = 0;
    ...
    // Use enter to go to functions rather than pressing the button for the thingy
    public void processEnter() {
        ...
        case PASSWORD:
            ...
            break;
        case LOGGED_IN:
            // If a user is logged in and enter is pressed, then the program checks what int
            // funcNum is assigned and carries out the associated task to the int
            switch (funcNum) {
            // Withdraw == (funcNum = 1)
            case 1:
                ...
                // Password Change == (funcNum = 2)
            case 2:
                if (state.equals(LOGGED_IN)) {
                    bank.changePassword(number);
                    display2 = "Password changed to:" + number;
                    number = 0;
    ...
    // Just a method to ask the user if about seeing balance. Added in order to
    // make sure the user can be asked before the withdrawal is complete.
    public void processBalCk() {
        display2 = "Would you like to view your balance before withdrawing?";
        display();
    }

    // Combined Balance and withdrawal method - Checks to see if the user is logged
    // in
    // if so offer them their balance and request withdrawal amount

    public void processBalWith() {
        if (state.equals(LOGGED_IN)) {
            display2 = ("Balance: " + bank.getBalance() + "   How much would you like to withdraw?");
            number = 0;
            funcNum = 1;
            display1 = "";
        } else {
    ...
    // Withdraw button - check we are logged in and if so ask user how much they
    // would like to withdraw
    // (number is the amount showing in the interface display)
    public void processWithdraw() {
        if (state.equals(LOGGED_IN)) {
            display2 = ("How much would you like to withdraw?");
            funcNum = 1;
            display(); // update the GUI
        } else {
            display2 = ("You are not logged in!");
            display();
        }
    }

    // Deposit button - check we are logged in and if so ask how much you want to
    // deposit
    // (number is the amount showing in the interface display)
    public void processDeposit() {
        if (state.equals(LOGGED_IN)) {
            display2 = ("How much would you like to deposit?");
            funcNum = 3;
            display();
        } else {
            display2 = ("You are not logged in!");
    ...
    // chgPS button - Change Password Request
    public void processPasswordChange() {
        if (state.equals(LOGGED_IN)) {
            display2 = ("What would you like to change your password to?");
            funcNum = 2;
        } else {
            initialise("You are not logged in");
        }
